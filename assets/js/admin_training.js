...
function createTableRow(user, index) {
  return `
      <tr data-user-id="${user.id}">
          ...
          <td>${data.site}</td>
          <td>
                <td><button class="btn btn-primary viewBtn" data-bs-toggle="modal" data-bs-target="#viewModal">View</button></td>
          </td>
      </tr>
  `;
}
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Function to add "View" button functionality
  function attachViewButton(row) {
    const viewButton = row.querySelector('.viewBtn');
    viewButton.addEventListener('click', function () {
      const trainingID = row.cells[0].innerText;
      const trainingTitle = row.cells[1].innerText;
      ...
      const agency = row.cells[6].innerText;
      const site = row.cells[7].innerText;

      // Populate modal fields
      document.getElementById('viewTrainingID').innerText = trainingID;
      document.getElementById('viewTrainingTitle').innerText = trainingTitle;
      document.getElementById('viewStartDate').innerText = startDate;
      document.getElementById('viewEndDate').innerText = endDate;
      document.getElementById('viewHours').innerText = hours;
      document.getElementById('viewType').innerText = type;
      document.getElementById('viewAgency').innerText = agency;
      document.getElementById('viewSite').innerText = site;
    });
  }

  // Add event listener for existing rows
  document.querySelectorAll('tbody tr').forEach(row => attachViewButton(row));
  ...
  function addRow(data) {
    ...
    // Attach view button functionality to the new row
    attachViewButton(newRow);
  }
